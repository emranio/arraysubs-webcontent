# Page Plan: Refund Policy

> **URL**: `/refund-policy/`
> **Tier**: T7 — Trust & Legal
> **Priority**: Week 1
> **Template**: `default`
> **Last Reviewed**: April 2026

---

## SEO Meta

| Field | Value |
|-------|-------|
| **Title Tag** | Refund Policy — 60-Day Money-Back Guarantee | ArraySubs |
| **H1** | 60-Day Money-Back Guarantee |
| **Meta Description** | ArraySubs Pro offers a full 60-day no-questions-asked refund. Submit the form, get your money back automatically. No hoops to jump through. |
| **Canonical** | `https://arraysubs.com/refund-policy/` |
| **Index** | Yes |

---

## Overview

**Sold by:** ArrayHash  
**Owner:** Emran  
**Contact:** emran@arrayhash.com  
**Effective Date:** April 10, 2026

ArraySubs Pro is sold with a **60-day full refund guarantee**. We believe in the product — if it is not right for you, you should not be stuck with it.

---

## Refund Terms

### Eligibility

- **Product**: ArraySubs Pro (all license tiers)
- **Window**: 60 calendar days from the date of purchase
- **Amount**: 100% of the purchase price — no partial refunds, no restocking fees
- **Condition**: No conditions required. We do not require a reason.

### What Is Not Refundable

| Item | Reason |
|------|--------|
| Purchases older than 60 days | Outside the guarantee window |
| Renewals already used for the active period and past 60 days from renewal date | Consumed service |
| Chargebacks or disputes filed with your bank before requesting a refund | See note below |

> **Note on chargebacks**: Filing a chargeback before requesting a refund may result in suspension of the associated licence. Please contact emran@arrayhash.com first — we always resolve issues directly and promptly.

---

## How to Request a Refund

The refund process is **fully automatic**:

1. Navigate to the refund request form at **`arraysubs.com/refund-policy/`** (form embedded on this page)
2. Fill in: order email address, order number, and purchase date
3. Submit the form
4. Refund is processed immediately — no manual review, no approval wait
5. Credit appears on your original payment method within **5–10 business days** depending on your bank or payment provider

You do **not** need to justify your request, uninstall the plugin, or contact support first.

---

## Automatic Processing

Refunds are triggered programmatically upon valid form submission within the 60-day window. No human intervention is required. You will receive an email confirmation once the refund has been issued.

---

## After a Refund

- Your Pro licence key is deactivated automatically upon refund
- The free version of ArraySubs remains available from the WordPress plugin repository at no charge
- You are welcome to repurchase at any time

---

## Renewal Refunds

If you were charged for a licence renewal and decide the product is no longer needed:
- Renewals are eligible for a refund within **60 days of the renewal charge date**
- Same process: submit the form with the renewal order number

---

## Exceptional Circumstances

If a technical issue with our payment processor prevents the automatic refund from completing, email **emran@arrayhash.com** with your order number and we will process it manually within 2 business days.

---

## EU / UK Consumer Rights

If you are an EU or UK consumer, you also have statutory rights including a 14-day cooling-off period under the EU Consumer Rights Directive and the UK Consumer Contracts Regulations. Our 60-day policy exceeds these statutory minimums.

---

## Contact

**ArrayHash**  
Email: emran@arrayhash.com

For refund issues only — the form on this page is faster. Email is a fallback.

---

## Content Blocks (Page Layout)

1. **Hero / Guarantee badge** — "60-Day Money-Back Guarantee. No questions asked."
2. **How it works** — 3-step visual: fill form → submit → refund issued automatically
3. **Refund form** — embedded form (order email, order number, purchase date, submit button)
4. **Full policy terms** — eligibility, window, exclusions
5. **After your refund** — licence deactivation, free version still available
6. **EU/UK statutory rights note**
7. **Contact block** — emran@arrayhash.com as fallback

---

## Key Messaging

- "No questions asked"
- "Fully automatic — no waiting for approval"
- "60 days from purchase — not 30"
- "Credit back to your original payment method"

---

## Internal Links

| Target | Context |
|--------|---------|
| `/plans/` | Pricing CTA after policy |
| `/privacy-policy/` | Footer cross-link |
| `/terms-of-service/` | Footer cross-link |
| `/contact/` | Fallback support contact |
