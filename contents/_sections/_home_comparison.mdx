...
export const renderComparisonCell = (value) => {
  if (value === true) {
    return <Icon name="check" size={18} className="comparison-table__check" />;
  }

  if (value === false) {
    return <Icon name="x" size={18} className="comparison-table__x" />;
  }
      ...
      <tbody>
        {comparisonRows.map((row) => (
          <tr key={row.feature}>
            ...
            <td className="comparison-table__cell">{renderComparisonCell(row.free)}</td>
            <td className="comparison-table__cell">{renderComparisonCell(row.pro)}</td>
            <td className="comparison-table__cell">{renderComparisonCell(row.woo)}</td>
          </tr>
        ))}
